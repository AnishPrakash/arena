...
**A remote code execution and judging service for competitive programming contests.**
Submit source code; Arena compiles it, runs it against hidden tests inside a hardened
sandbox, measures CPU time and peak memory deterministically, and ranks participants on a
live leaderboard.
...
**Live demo:** https://arena.<your-domain>.com · **Demo video:** [5-minute walkthrough](<drive-link>)
**Login:** `anish` / `password123`

**Build status (29 Aug 2026):** Phases 0–8 complete and pushed. `make smoke` **11/11**,
golden judge-the-judge suite **18/19**, `go test ./... -race` green, architecture rules
enforced in CI. Load-tested to a **7,371-job backlog** with submission p95 at **27.4 ms**
and zero 5xx. Known gaps are listed in §16 — none of them are silent.
...
cp .env.example .env
make images && make up     # postgres, redis, api, seeder, 2 runners, prometheus, grafana
make smoke                 # asserts an 11-case verdict matrix end to end
```
...
- **Executing untrusted code** — seconds of pinned CPU per submission, hostile input,
  hard to isolate, expensive.

Running them in the same process means your HTTP worker pool *is* your execution capacity.
At contest start every request blocks for seconds, the pool saturates, clients time out on
submissions that actually ran, and nobody can tell a participant what happened to their
code.
...
instead of dropped requests. The API answers in under 20 ms regardless of how deep the
judging backlog is. Runner capacity scales on its own signal, on its own hardware, at its
own cost.

Every other design decision in this document follows from that one.
...
flowchart TB
    ...
    subgraph DATA["STATE"]
        ...
        BLOB[("Object store<br/>source blobs · test data")]
    end
...
somewhere that cannot touch participant data. In Kubernetes a `NetworkPolicy` additionally
blocks egress to Postgres and to `169.254.169.254` (cloud metadata / IAM credentials).
...
sequenceDiagram
    ...
    R->>S: compile ONCE (own limits, cached)
...
pending entries list and another runner reclaims it after the lease expires. Results are
...
flowchart TB
    subgraph C["Sandbox container — created per submission, destroyed after"]
        ...
        L2["2 · cgroups v2 — memory.max=256M · swap OFF · pids.max=64 · cpu.max"]
...
Each layer assumes the one above has been bypassed. A seccomp bypass still faces cgroups;
...
asynchronously and reports the container, not the program. So Arena ships a ~200-line
static Go binary into every language image. It is PID 1 in the sandbox, applies rlimits,
execs the participant's program, kills the whole **process group** on a wall deadline, and
reports exact usage from `wait4(2)`.
...
1. **`--memory-swap` must equal `--memory`.** Docker's default is `2 × memory` of swap, so
   a leaking program swaps instead of OOMing: a clean `MLE` degrades into a 60-second wall
   timeout that also thrashes every other submission on the node.
2. **cgroup memory, not `RLIMIT_AS`.** The JVM and Node reserve huge *virtual* address
   space at startup; an address-space limit fails them spuriously. cgroups measure RSS.
...
5. **Separate, larger compile limits.** g++ with heavy templates legitimately needs ~512 MB.
   Reusing run limits produces phantom `CE`s that look like judge bugs.

### On seccomp — a deliberate non-decision

Docker applies a curated seccomp **whitelist** by default, blocking ~44 syscalls including
`ptrace`, `mount`, `pivot_root`, `bpf` and `kexec_load`. Passing a custom profile
*replaces* it rather than adding to it, so a hand-written denylist
(`defaultAction: SCMP_ACT_ALLOW` plus a few `ERRNO` entries) would be strictly **weaker**
while looking more rigorous. Arena keeps the stronger default and layers `cap-drop=ALL`,
`no-new-privileges`, non-root uid and a read-only rootfs on top. The documented upgrade is
gVisor (`--runtime=runsc`), which is a one-line change behind the `Sandbox` interface.

---

## 4. Handling the four disruption classes
...
| **Dead or preempted runner** | Redis Streams lease + `XAUTOCLAIM` redelivery; `SIGTERM` nacks in flight | `internal/adapters/redisq` |
...
| **Poison pill** | delivery-count dead-lettering after `MAX_ATTEMPTS` | `internal/adapters/redisq` |
...
**Security rule:** program stdout is returned only for *sample* tests. Echoing it for
hidden tests would let anyone exfiltrate the entire test set with
...
byte-identically. CI tags images by commit SHA, so the recorded digest always resolves.
...
|---|---|---|---|
| 1 slot per dedicated core (shipped default) | `[__] ms` | `[__] ms` | **`[__]%`** |
| 4 slots per core (oversubscribed) | `[__] ms` | `[__] ms` | `[__]%` |

> **Methodology note.** The first run of this harness reported a coefficient of variation
> of exactly 0.00% across 20 samples — not determinism, but the verdict cache replaying a
> single stored result nineteen times. `scripts/determinism.sh` now appends a unique
> comment to each iteration's source so every sample actually executes. Any determinism
> figure produced without that guard is measuring a hash lookup.
...
brief asks for. That trade-off is the reason the numbers above are in this README.
...
neighbours — strictly better than milliseconds for ranking algorithmic efficiency. It is
not enabled because the syscall is in Docker's default seccomp denylist and most cloud VMs
expose no virtualised PMU. The metric field exists in the schema and the scoring function
prefers it when present; see `DECISIONS.md` ADR-009.
...
Runners are **never** scaled on CPU: a judge node is *supposed* to sit at 100% CPU, so a
CPU trigger either never scales up or never scales down.
...
- **Fair scheduling.** A per-user token bucket *plus* a one-in-flight-job lock. Rate
  limiting alone does not stop a participant with 12 queued submissions from occupying 12
  judging slots in the final two minutes while everyone else waits.
- **Bulkheads.** A consumer group per contest, so one event's flood cannot starve another.

**Admission control:** past a backlog threshold the API returns `429` with `Retry-After`
and shows the estimated wait. Honest degradation beats silent timeouts.

### Measured under load (`scripts/load.js`, ramping arrival rate)
...
| 5xx responses | **0** |
| 429 responses (correct behaviour under load) | 13,950 in the unthrottled run |
...
7,371 deep.** That is the entire reason the queue exists — the burst became an observable
number instead of dropped requests.

Two things the first two runs taught us, both worth stating because they are the difference
between a load test and a load *measurement*:

1. **The first run measured the rate limiter, not the system.** k6 hammered as a single
   authenticated user, so the per-user token bucket capped intake at ~1.15 submissions/s
   and returned 13,950 `429`s. Correct behaviour, useless data. The harness now registers
   60 distinct users.
2. **The second run measured the verdict cache.** Four distinct source files across
   thousands of iterations meant 7,825 submissions were served from cache and the backlog
   never left zero. `scripts/load.js` now has a `UNIQUE=1` mode that appends a per-iteration
   comment to defeat the cache, which is how the 7,371 backlog above was produced.
...
| Postgres (Neon free) + Redis (Upstash free) + R2 (zero egress) | $0 |
...
scale-to-baseline between contests → ARM → node-local testdata cache with zero-egress
object storage.
...
Plus one Dockerfile. **Zero Go changes** — `internal/langs/registry_test.go` asserts this
by registering a brand-new language from bytes alone.

`time_multiplier` and `memory_overhead_mb` neutralise the fixed startup tax of interpreted
and JIT runtimes (Python ×4, Java ×2.5 + 128 MB), so an identical algorithm gets an
identical verdict across languages. Fairness policy is **data**, not code, and it is
applied once at submission time so the runner never has to know about it.

### Ports and adapters

`internal/core` holds verdict classification, limit arithmetic and scoring with **zero
I/O** — it imports only the standard library, so its tests run in milliseconds without
Docker or Postgres. Everything else is an adapter behind an interface:
...
Swapping Docker for gVisor, Redis Streams for SQS, or local blobs for S3 is an adapter, not
a rewrite. Dev and CI run a `process` sandbox with no Docker daemon; production refuses to
build it (`ARENA_ENV=prod` rejects the driver outright).

**The layering is enforced, not asserted.** `scripts/check-arch.sh` runs in CI and fails
the build if `internal/core` ever imports an adapter, `net/http`, `pgx` or `go-redis`, or
if two adapters import each other.
...
- **The leaderboard is never a `GROUP BY` over `submissions`.** It is a Redis ZSET updated
  per verdict by a Lua script, with `(solved, penalty)` packed into one `float64` so ranking
  is O(log n). Postgres is the durable source of truth used only to rebuild it — a live
  aggregate query over the submissions table is the classic contest-day outage.
- **Migrations are embedded** (`embed.FS`) and applied on boot under a `pg_advisory_lock`,
  so a rolling deploy of N replicas is safe and there is no CLI to install between clone and
...
| Client IP spoofing | chi's `middleware.RealIP` deliberately **not** used (GHSA-3fxj-6jh8-hvhx): it trusts `X-Forwarded-For` unconditionally. Rate limits are per authenticated user, never per IP |
...
**Accepted risk, stated plainly:** the runner mounts the Docker socket, which is equivalent
to root on its node. That is acceptable because the runner is the *trusted* component —
...
`testdata/golden/` holds 19 fixture programs whose correct verdict is known in advance, run
in CI on every push. Current result: **18/19 passing** — the one failure is a known fixture
bug, not a judge bug, and is recorded in §16.

| Category | Cases |
|---|---|
| Correctness | AC in Python and C++, WA |
| Limits | busy loop (`RLIMIT_CPU`), **`sleep(600)`** (wall clock only — the case a single-timeout judge gets wrong), O(n²) on 200k input, one-shot allocation, **gradual leak** (proves swap is disabled), div-by-zero, segfault, stack overflow, output flood, compile error |
...
Plus: table-driven unit tests on verdict precedence and limit arithmetic, an integration
test proving **lease redelivery after a consumer dies**, a poison-pill dead-lettering test,
...
The distribution is deliberate. `internal/core` is where a wrong line becomes a wrong
verdict, so it is table-tested exhaustively and runs in milliseconds with no Docker and no
Postgres. The adapters are verified against **real** Postgres and Redis (`-tags=integration`)
and by the golden suite driving the full submit → queue → sandbox → verdict path. Mocking a
sandbox would raise the number and prove nothing about whether `RLIMIT_CPU` actually fires.
...
| A leaking program timed out instead of hitting `MLE` | 60 s wall kills, host thrashing | Docker defaults `--memory-swap` to `2 × memory`, so the program swapped | set `--memory-swap == --memory`; a golden fixture (`mle_growlist.py`) now guards it |
| `sleep(600)` was never caught | submission hung until the outer timeout | `RLIMIT_CPU` counts CPU, and sleeping burns none | added an independent wall-clock watchdog that kills the process group |
...
| JVM/Node failed instantly under a memory limit | every Java submission was `MLE` | `RLIMIT_AS` counts *virtual* address space, which JITs reserve enormously | dropped `RLIMIT_AS` entirely; memory is a cgroup concern (RSS) |
...
| Correct answers marked `WA` | only on Windows clients | trailing `\r\n` | token-based comparison as the default checker |
...
| Two Compose runner replicas fought over jobs | crossed heartbeats, stolen work | identical consumer name in the Redis group | entrypoint derives `ARENA_RUNNER_ID` from the container hostname |
| Timings varied ±25% run to run | unusable for ranking | judging slots oversubscribed the cores | slots capped at `cores − 1`, each pinned with `--cpuset-cpus`; measured in §5 |
| **Verdicts attributed to the wrong submission** | golden suite 10/19, failures moved between runs | slot directories were `<box>/slot-N`, and slot numbers are *per-runner* — two runners on one host both resolved slot 0 to the same directory and overwrote each other's source, stdin, stdout and `meta.json` | the runner id is now part of the path: `<box>/<runner-id>/slot-N`. Suite went 10/19 → 17/19 and wall time 95 s → 13.6 s |
| Every C++ submission was `CE` | `ld terminated with signal 25` in the compile log | the compile step inherited the run step's tight `RLIMIT_FSIZE`; that rlimit bounds *every* file a process writes, so it killed the linker while it was writing the output binary | a separate compile envelope with a 64 MiB `FSIZE` (`core.DefaultCompileLimits`) |
| The C++ fix appeared not to work | identical `CE` after a correct fix and a rebuild | the verdict cache replayed the stale pre-fix result — its key covers source, testdata and image, but **not the limits** | cache flushed; the key is a known gap, recorded in §16 |
...
| Reclaimed jobs failed as `IE` | after a runner was SIGKILLed | the orphaned sandbox container still held the name the retry wanted | containers are labelled `arena.runner=<id>`, removed before create, and swept at startup (`SweepStale`) |
| The reconciler re-enqueued forever | a stuck submission cycled indefinitely | the attempt counter was incremented in memory but never persisted | `BumpAttempt` writes it before re-publishing; beyond `MAX_ATTEMPTS` the submission fails honestly as `IE` |

---

## 13. Tech stack, with the alternative rejected
...
Full reasoning with context and consequences: **[`DECISIONS.md`](./DECISIONS.md)**.
...
Every knob is an environment variable with a documented default — see `.env.example`.
The ones that matter:

| Variable | Default | Why it matters |
|---|---|---|
| `ARENA_RUNNER_SLOTS` | `2` | **Set to `cores − 1`.** Raising it above the core count destroys timing determinism |
...
- **Live deployment is single-node.** The Kubernetes + KEDA manifests in `deploy/k8s` and
  `deploy/keda` are written and reviewed but not running; they are the documented scale-out
  path, not a claim about the demo.
- **gVisor / Firecracker isolation** — designed for behind the `Sandbox` interface,
  not enabled. Docker + cgroups + the default seccomp profile is what ships.
- **Instruction counting** — the schema field, the scoring preference and the capability
  probe exist; the PMU is unavailable on the target platform (ADR-009).
- **Special judges and interactive problems** — the `Checker` registry is designed for
  them; only `exact`, `token` and `float` are implemented.
- **Table partitioning on `submissions`** — the DDL is in `DECISIONS.md`; it complicates
  unique constraints and buys nothing at hackathon scale.
- **Warm container pool** — pre-created paused containers would take start-up from ~250 ms
  to ~50 ms. Image pre-pull is implemented; the full pool is not.

### Known gaps, measured rather than assumed

These are open. They are listed because a reviewer who finds them here trusts the rest of
the document more than one who finds them in the code.

- **The verdict cache key omits the limit envelope** (ADR-016). It covers `source_hash`,
  `testdata_version` and `image_digest`, so changing a problem's time or memory limit does
  not invalidate cached verdicts. This bit during development (§12) and the fix is one
  field in the key; it is not applied because doing so at this point would invalidate the
  measurements in §5 and §6.
- **The queue claim batch is bound to configured slots, not to free slots.** Each
  `XREADGROUP` claims `len(slots)` messages, and `XAUTOCLAIM` re-claims keep entries in the
  Pending Entries List rather than draining it. Under sustained load the PEL was observed at
  **5,822 entries against a 702-job backlog**. Nothing is lost — the heartbeat renews the
  leases and reclaim recovers them — but it widens the redelivery window after an ungraceful
  kill. Bounding the batch to genuinely free slots is a one-line change in `Consume`.
- **One golden fixture fails (18/19).** `quadratic-on-large-input` declares a
  `problem_override` for the time limit that the runner does not honour, so the case is
  judged against the default envelope. The fixture is wrong, not the judge — but it is
  failing, and it is counted as failing.
- **Instruction counting is unavailable on the target.** `PERF_COUNT_HW_INSTRUCTIONS` was
  probed and returns nothing under WSL2/Hyper-V (no virtualised PMU), and the syscall sits
  in Docker's default seccomp denylist. See ADR-009.
...
deploy/         compose · Dockerfiles · k8s · keda · prometheus · grafana
...
make seed          # 3-problem demo contest
make images        # language sandbox images
make golden smoke load
```

Requires Linux (or WSL2) with cgroups v2, Docker, Go 1.25+. See `docs/` for the full
build walkthrough.
